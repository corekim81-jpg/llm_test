...
PROMQL: dict[str, dict[str, str]] = {
    "web": {
        ...
        # apache_exporter
        "req_per_sec": ('rate(apache_accesses_total{instance="{instance}"}[1m])'),
        "bytes_per_sec": (
            'rate(apache_sent_kilobytes_total{instance="{instance}"}[1m]) * 1024'
        ),
        "workers_busy": ('apache_workers{instance="{instance}",state="busy"}'),
        "workers_idle": ('apache_workers{instance="{instance}",state="idle"}'),
    },
    "was": {
        ...
        "threads_active": ('jvm_threads_current{instance="{instance}"}'),
        "threads_daemon": ('jvm_threads_daemon_current{instance="{instance}"}'),
        "gc_time_rate": (
        ...
        "req_per_sec": ('rate(tomcat_requestcount_total{instance="{instance}"}[1m])'),
        "error_per_sec": ('rate(tomcat_errorcount_total{instance="{instance}"}[1m])'),
...
if __name__ == "__main__":
    ...
    test_cases = [
        ("dev-masternode",   "web", "bank-web-hostmetrics"),
        ("ONTUNETEST2",      "was", "bank-was-hostmetrics"),
        ("DESKTOP-H0M89JB",  "db",  "bank-db-hostmetrics"),
    ]
        ...
        if data.get("ok"):
            ...
            for name, val in metrics.items():
                if isinstance(val, dict) and "avg" in val:
                    print(
                        f"  {name:<30} avg={val['avg']:<8} max={val['max']:<8} p95={val['p95']}"
                    )
