...
from monitoring_llm.nlp.intent_classifier import IntentClassifier, QueryIntent, ClassifyResult
from monitoring_llm.nlp.time_parser import parse_time_expression, default_range, TimeRange
from monitoring_llm.nlp.param_binder import bind_params, BoundParams
...
if __name__ == "__main__":
    ...
    CONVERSATION = [
        ("어제 어떤 서버에 문제가 있었어?",                 {}),
        ("was01 서버 메트릭 보여줘",                       {}),
        ("그 서버 로그도 같이 확인해줘",                   {"current_servers": [
            {"hostname": "was01-bank16", "ip": "192.168.16.20",
             "role": "was", "prometheus_instance": "192.168.16.20:9090",
             "loki_host": "was01-bank16", "prometheus_job": "jmx_exporter"}
        ]}),
        ("이 상황에서 어떤 조치를 취해야 해?",              {"current_servers": [
            {"hostname": "was01-bank16", "ip": "192.168.16.20",
             "role": "was", "prometheus_instance": "192.168.16.20:9090",
             "loki_host": "was01-bank16", "prometheus_job": "jmx_exporter"}
        ]}),
    ...
    for i, (text, state) in enumerate(CONVERSATION, 1):
        ...
        if bp.loki_keyword or bp.loki_status_code:
            print(f"  loki       = keyword={bp.loki_keyword}, code={bp.loki_status_code}")
