...
TRACE_ID_RE    = re.compile(r"(?:traceId|trace_id|TraceID)[=:\s]+([0-9a-f]{16,32})", re.IGNORECASE)
...
class LogAnalysisResult:
    ...
    def to_prompt_text(self, max_logs: int = 10) -> str:
        ...
        if self.patterns:
            pat_str = ", ".join(f"{k}×{v}" for k, v in
                                sorted(self.patterns.items(), key=lambda x: -x[1])[:5])
...
def preprocess(loki_json: str | dict) -> LogAnalysisResult:
    ...
    if "key_logs" in data:
        ...
        res.level_counts = data.get("level_counts", {})
...
# ── 편의 함수 ─────────────────────────────────────────────────────────
def analyze_loki_result(loki_json: str | dict, use_llm: bool = False) -> LogAnalysisResult:
...
if __name__ == "__main__":
    ...
    SAMPLE_LOKI = {
        ...
        "key_logs": [
            ...
            {"time": "14:32:03", "level": "ERROR",
             "log": '[ERROR] "POST /api/transfer HTTP/1.1" 500 1234'},  # 중복
